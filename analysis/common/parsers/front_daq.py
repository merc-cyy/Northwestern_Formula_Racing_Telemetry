...
@parser_class(ParserVersion("FrontDAQ", 0))#thsi calss is a aparser and its version 0
class FrontDAQParser(BaseParser):#the pasring implementation that takes from the base template of BaseParser
    @staticmethod
    def parse(filename: str) -> CarDB:#takes in the binary file we want to read and returns it as a CarDB
        ...
        total_bytes = os.path.getsize(filename)#total bytes in the file
        if total_bytes % record_size != 0:# total / size of one record == a whole number or else we have some incomplete records
            raise ValueError(
                f"'{filename}' is {total_bytes} bytes, "
                f"not a multiple of {record_size}"
            )
        n_records = total_bytes // record_size#number of records from data
